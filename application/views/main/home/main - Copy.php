...
<div class="container-fluid" style="background:#b7d034; margin-bottom:30px;" >
...
<div class="container" style="background: white; width:auto;">
	...
	<div class="col-md-4">
		...
		<div class="panel panel-default" style="border-radius:0px;border:none !important;box-shadow:none !important;">
				...
				<div class="panel panel-default" style="border-radius:0px;border:none !important;box-shadow:none !important;">
					<?php 
						$now=new DateTime();
						$promoQuery=$common->queryRow("SELECT tgl_promo,judul,isi_promo FROM rs_promo WHERE tgl_berlaku_promo > ".$now->format('Y-m-d')." ORDER BY tgl_promo ASC LIMIT 1");
						$tanggalPromoIndo='';
						if($promoQuery){
							$datePromo=new DateTime($promoQuery->tgl_promo);
							$tanggalPromoIndo='<font style="color:#777;font-size: 10px;"><i>'.$datePromo->format('d M Y').'</i></font>';
						}
					?>
					<div class="panel-heading" style="background:none;">
						<h2 class="panel-title">Promo Rumah Sakit <?php echo $tanggalPromoIndo; ?></h2>
					</div>
					<div class="panel-body">
						<?php 
						...
						if($promoQuery){
							...
							echo $common->stringLimit($promo->isi_promo,100);
						?>
						<br><br>
	...
	<ul class="ul-custom">
	<?php 
		$unitAbouts=$common->queryResult("SELECT judul,description,image_name,A.unit_id,A.unit_name,A.unit_type
				FROM rs_unit_about M INNER JOIN rs_unit A ON M.unit_id=A.unit_id ORDER BY RAND() LIMIT 3");
		if($unitAbouts){
			for($i=0,$ilen=count($unitAbouts); $i<$ilen ; $i++){
				$unitAbout=$unitAbouts[$i];
				$unitImage=	'NO.GIF';
				if($unitAbout->image_name != null && $unitAbout->image_name != '')
					$unitImage=$unitAbout->image_name;
	?>
	<li style="padding: 10px;" class="row" onMouseOver="loadCaption('1','1')" onMouseOut="loadCaption('0','1')">
    <div class="col-md-4">
    <img class="img-responsive" style="border: 10px solid white; box-shadow: 0px 10px 10px -10px rgba(0, 0, 0, 0.7);" src="<?php echo base_url().'upload/'.$unitImage; ?>" />
     <div class="desc" id="capt-1">
            <p class="desc_content"><?php echo $unitAbout->judul; ?>.</p>
        </div>
        </div>
                <div class="col-md-8">
                <div class="panel panel-default" style="border:none; background:none; box-shadow:none; text-align:right;">
                <div class="panel-heading">
        <div class="panel-title"><h4><?php echo $unitAbout->judul; ?></h4></div>
        </div>
        <div class="panel-body">
        <?php echo $common->stringLimit($unitAbout->description,200); ?>
        <p>
        </div>
        <a href="<?php echo base_url() ;?>pelayanan/pelayananrs?page=<?php echo $unitAbout->unit_id; ?>&title=Penunjang Medis&sub_title=<?php echo $unitAbout->unit_name; ?>&type=<?php echo $unitAbout->unit_type; ?>" class="btn btn-info">Selengkapnya >></a>
        </div>
        </div>
    </li>
	<?php 
