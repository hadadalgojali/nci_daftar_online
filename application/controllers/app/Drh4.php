...
//Kecamatan
class Drh4 extends MY_controller {
	public $MA='DRH4';
	...
	public function getList(){
		...
		$entity=$common->getModel('Districts');
	...
	public function getCity(){
		...
		$res=$common->createQuery("SELECT u FROM ".$common->getModel('District')." u
	...
	public function save(){
		...
		$parent=$common->find('District',$this->post('f1'));
		$name=$this->post('f2');
		if($pageType=='ADD'){
			$o=$common->newModel('Districts');
			$o->setValue($name)
				->setParent($parent)
				->save();
			$result->setMessageSave ('Districts Name ', $name )->end ();
		}else{
			$o=$common->find('Districts',$pid);
			if ($o != null) {
				$o->setValue($name)
					->update();
				$result->setMessageEdit ('Districts Name ', $name )->end ();
			}else
				$result->error()->setMessageNotExist()->end();
		}
	}
	public function delete(){
		...
		$code='';
		$res= $common->find('Districts',$pid);
		if ($res != null) {
			$code=$res->getValue();
			$res->delete();
		}else
			$result->error()->setMessageNotExist()->end();
		$result->setMessageDelete('Districts Name ', $code )->end ();
	}
}
