...
// 			$oM['code']=$oO->getUnitCode();
	...
	public function generateSEP($noRegistrasi,$baru,$medrec){
		...
		if($no_sep->metaData->code=='201'){
			...
			return array('true',$no_sep->response->sep->noSep);
		}
	}
	...
	public function getJsonReq($getJsonReq,$baru,$medrec){
		...
		$noSurat ='000056';
		$kodeDPJP='12708'; // HARUSNYA DIAMBIL DARI INPUTAN USER [dokter perujuk]
		$json='{
           "request": {
              "t_sep": {
	...
	public function save_bpjs_pasien_lama($noRegistrasi,$baru,$medrec_lama,$unit,$customer,$ori){
		...
		if($cetak_sep[0] == 'true'){
			...
			$insert_sjp_kunjungan="INSERT INTO sjp_kunjungan 
				...
				VALUES ('$medrec_lama','$unit','$now','$retVal','$cetak_sep')";
	...
	public function save_bpjs_pasien_baru($noRegistrasi,$baru,$unit,$customer,$ori,$medrec_lama){
		...
		//	echo $medrec;
			...
			if($cetak_sep[0] == 'true'){
				...
				//insert sjp kunjungan 2018-11-29
				$insert_sjp_kunjungan="INSERT INTO sjp kunjungan 
						(kd_pasien,kd_unit,tgl_masuk,urut_masuk,no_sjp)
					VALUES ('$medrec','$unit','$now','$retVal','$cetak_sep')";
				$db->query($insert_sjp_kunjungan);
