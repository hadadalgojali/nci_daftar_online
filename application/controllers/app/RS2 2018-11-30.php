...
class RS2 extends MY_controller {
	...
	*/
	...
	public function getDokter(){
		...
		$res=$common->createQuery("SELECT B.id,B.firstName,B.lastName FROM ".$common->getModel('DokterKlinik')." u
			INNER JOIN u.unit A
			INNER JOIN u.dokter B
...
// 			$oM['code']=$oO->getUnitCode();
	...
	//	echo $jenis_pasien;
		...
		if($res->getResult()){
			...
			//---KUNJUNGAN BARU ATAU LAMA (keliru untuk menentukan pasien lama baru dari kunjungan)
			/*$kunj=$db->query("select * from kunjungan where kd_pasien='".$medrec_lama."' ");
			if ($kunj->num_rows() == 0) {
				$baru = 'true';
			} else {
				$baru = 'false';
	...
	public function dataList($type=false){
		...
		switch ($sorting){
			...
			case "f11":
				...
				break;
			default:
				$orderBy.='B.unitName,C.firstName,M.entryDate,M.antrian ASC';
				break;
		}
	...
	public function generateSEP($noRegistrasi,$baru,$medrec){
		...
		$getJson=$this->getJsonReq($get_data_bpjs,$baru,$medrec);
	...
	public function getJsonReq($getJsonReq,$baru,$medrec){
		$no_medrec='';
		$db=$this->load->database('other',true);
		if($baru=='false'){
			$no_medrec=$medrec;
	...
	public function cetaksepbpjs($no_sep){
		...
		if($res->metaData->code=='200'){
			if($res->response->peserta->kelamin == 'Perempuan'){
				$jk='WANITA';
			} else{
				$jk='PRIA';
			} 
	...
	public function cetak_antrian(){
		$printer = 'localhost/EPSONLX_310_MAYA';
	...
	public function save_bpjs_pasien_lama($noRegistrasi,$baru,$medrec_lama,$unit,$customer,$ori){
			...
			$sql="INSERT INTO kunjungan (kd_pasien,kd_unit,tgl_masuk,urut_masuk,jam_masuk,cara_penerimaan,asal_pasien,kd_rujukan,	no_sjp,kd_dokter,baru,kd_customer,shift,karyawan,kontrol,antrian,online,anamnese)
						VALUES('$medrec_lama','$unit','$now','$retVal','$JamKunjungan',99,'$asal_pasien',0,'$cetak_sep','000','$baru',
	...
	public function save_bpjs_pasien_baru($noRegistrasi,$baru,$unit,$customer,$ori,$medrec_lama){
			...
			$cetak_sep=$this->generateSEP($noRegistrasi,$baru,$medrec);
			$sqlbagianshift = $db->query("SELECT   shift FROM BAGIAN_SHIFT  where KD_BAGIAN='2'")->row()->shift;
			$lastdate = $db->query("SELECT   to_char(lastdate,'YYYY-mm-dd') as lastdate FROM BAGIAN_SHIFT  where KD_BAGIAN='2'")->row()->lastdate;
			$datnow = date('Y-m-d');
			if ($lastdate <> $datnow && $sqlbagianshift === '3') {
		...
		//	print_r($get_pasien);
			...
			$insert_pasien_baru="INSERT INTO pasien (kd_pasien,nama,tgl_lahir)
			VALUES(
				 '$kd_pasien',
				 '$nama_pasien',
				 '$tgl_lahir_pasien'
				)";
			$db->query($insert_pasien_baru);
			$sql="INSERT INTO kunjungan (kd_pasien,kd_unit,tgl_masuk,urut_masuk,jam_masuk,cara_penerimaan,asal_pasien,kd_rujukan,no_sjp,kd_dokter,baru,kd_customer,shift,karyawan,kontrol,antrian,online,anamnese)
			VALUES('$medrec','$unit','$now','$retVal','$JamKunjungan',99,'$asal_pasien',0,'$cetak_sep','000','$baru','$customer',
	...
	public function save_pasien_baru_umum($noRegistrasi,$baru,$unit,$customer,$ori,$medrec_lama){
			...
			$asal_pasien=$db->query("select kd_asal from asal_pasien where kd_unit='$cari_kd_unit'")->row()->kd_asal;
			//$get_pasien=$this->db->query("select patient_code,name,birth_date from rs_patient where patient_code='$medrec_lama'")->result();
			$get_pasien=$this->db->query("select patient_id,patient_code,name,birth_date from rs_patient where patient_code='$medrec_lama'")->result();
...
//				$db->trans_commit();
			...
			$Data .= $condensed1;
			$Data .= $condensed;
			$Data .= $condensed.str_pad($bold1.'RSUD dr. Soedono'.$bold0,64," ",STR_PAD_LEFT)."\n";
		...
		//	echo $Data;
			fwrite($handle, $Data);
			fclose($handle);
			//copy($file, "//localhost/Foxit Reader PDF Printer"); # Lakukan cetak  //DONAT
			unlink($file);
