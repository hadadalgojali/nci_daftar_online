...
// 				</div>
	...
	public function getPatientList(){
		...
		// }
		$res=$common->queryResult("SELECT p.patient_id,p.patient_code,upper(p.name) as name FROM rs_patient p  WHERE (UPPER(p.patient_code) LIKE UPPER('%".$text."%') OR
			UPPER(name) LIKE UPPER('%".$text."%')) ".$criteria." ORDER BY p.patient_code ASC LIMIT 10");
		$arr=array();
		for($i=0,$iLen=count($res); $i< $iLen; $i++){
			$country=$res[$i];
			$o=array();
...
//$res= json_decode(file_get_contents('http://api.bpjs-kesehatan.go.id:8080/VClaim-rest/Rujukan//'.$no_kartu."/tglSEP/".date("Y-m-d"),false,$context));
//$res= json_decode(file_get_contents('http://api.bpjs-kesehatan.go.id:8080/VClaim-rest/Rujukan/Peserta//'.$no_kartu,false,$context));
	$res= json_decode(file_get_contents('https://new-api.bpjs-kesehatan.go.id:8080/new-vclaim-rest/Rujukan/Peserta//'.$no_kartu,false,$context));
    $data = array(
      'generate'  => $generate,
      'resp'      => $res,
    );
    /*if($data->resp->metaData->code=='200'){
    	echo "yeyy";
    }else{
    	echo "nopp";
    }*/
